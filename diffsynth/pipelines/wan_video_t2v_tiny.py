...
from ..models.wan_video_dit_t2v_tiny import WanModel_t2v_tiny
...
class WanVideoPipeline_t2v_tiny(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.float16, tokenizer_path=None):
        ...
        self.dit: WanModel_t2v_tiny = None
    ...
    def fetch_models(self, model_manager: ModelManager):
        ...
        self.dit = model_manager.fetch_model("wan_video_dit_lq_t2v_tiny")
    ...
    def from_model_manager(model_manager: ModelManager, torch_dtype=None, device=None, use_usp=False):
        ...
        pipe = WanVideoPipeline_t2v_tiny(device=device, torch_dtype=torch_dtype)
    ...
    def pad_input_lq(self, input_lq: torch.Tensor):
        # input_lq: Tensor of shape (B, C, F, H, W)
        B, C, F, H, W = input_lq.shape
        # print(f"input_lq.shape {F}")
        pad_frames = 0
    ...
    def test_tlc_seg(
        ...
        self.tlc_vae_latents = LocalAttention3D(tile_kernel, (0.95, 0.95, 0.95))
        self.tlc_vae_img = LocalAttention3D(tile_kernel, (0.95, 0.95, 0.95))
...
class TeaCache:
    ...
    def check(self, dit: WanModel_t2v_tiny, x, t_mod):
...
def model_fn_wan_video_tlc_test(
    ...
    **kwargs,
    ...
    frames = key_frame_idx[0][::4]  # 每 4 帧anchor 取一帧
    if frames[-1] != key_frame_idx[0][-1]:  # 如果最后一帧还没取到
        frames.append(key_frame_idx[0][-1])  # 补上最后一帧
