...
class BasePipeline(torch.nn.Module):
    ...
    def pad_spatial(self, video, patch_size=16):
        # input_lq: Tensor of shape (B, C, T, H, W)
        if video.dim() != 5:
            raise ValueError(f"Expected [B, C, T, H, W], got {tuple(video.shape)}")

        _, _, _, h, w = video.shape
        pad_h = (patch_size - h % self.height_division_factor) % self.height_division_factor
        pad_w = (patch_size - w % self.width_division_factor) % self.width_division_factor

        if pad_h > 0 or pad_w > 0:
            video = torch_F.pad(video, (0, pad_w, 0, pad_h, 0, 0), mode="replicate")
    ...
    def unpad_spatial(self, video, pad_h, pad_w):
        if pad_h > 0:
            video = video[:, :, :, :-pad_h, :]
        if pad_w > 0:
            video = video[:, :, :, :, :-pad_w]
    ...
    def pad_temporal(self, video):
        ...
        pad_t = target_t - t
        if pad_t > 0:
            video = torch.cat([video, video[:, :, -1:].repeat(1, 1, pad_t, 1, 1)], dim=2)   # 取最后一帧在时间维复制 pad_t 次
            print(f"Only `num_frames % 4 != 1` is acceptable. We round it up to {target_t}.")

        return video, pad_t


    def unpad_temporal(self, video, pad_t):
        if pad_t > 0:
            return video[:, :, :-pad_t]
        return video
