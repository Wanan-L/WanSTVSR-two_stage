#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Stage-2 TE training for WanSTVSR.

TE initializes DiT from a TC checkpoint and optimizes pixel L1, DISTS,
high-frequency optical-flow warp consistency, and MUSIQ quality losses. The
original paired-video validation path is retained for all requested metrics.
"""
...
class LightningModelForStage2(pl.LightningModule):
    ...
    def _load_initial_dit(self, checkpoint_path: str) -> None:
        ...
        if not isinstance(state_dict, dict):
            raise TypeError(f"Unsupported checkpoint state: {type(state_dict)}")
        ...
        missing_keys, unexpected_keys = self.pipe.dit.load_state_dict(dit_state, strict=False,)

        if missing_keys or unexpected_keys:
            raise RuntimeError(
                "Stage-1 DiT weights do not exactly match. "
                f"loaded={len(dit_state)}, "
                f"missing={len(missing_keys)}, "
                f"unexpected={len(unexpected_keys)}, "
                f"first_missing={missing_keys[:10]}, "
                f"first_unexpected={unexpected_keys[:10]}"
            )

        self._init_checkpoint_loaded = True
        py_logger.info(
            "Successfully loaded Stage-1 DiT: loaded=%d, model_total=%d, missing=0, unexpected=0, path=%s",
    ...
    def on_fit_start(self) -> None:
        if not self._init_checkpoint_loaded and not self.args.resume_from_checkpoint:
            raise RuntimeError("New TE training must use --init_from_checkpoint with a TC checkpoint.")
    ...
    def on_load_checkpoint(self, checkpoint: Dict[str, Any]) -> None:
        ...
        if source_stage != STAGE_NAME:
            raise ValueError(
                "Stage2 --resume_from_checkpoint requires a Stage2 checkpoint, "
...
def parse_args() -> argparse.Namespace:
    ...
    if not args.init_from_checkpoint and not args.resume_from_checkpoint:
        raise ValueError(
            "Start TE with --init_from_checkpoint <tc.ckpt>, or resume an "
            "interrupted TE run with --resume_from_checkpoint <te.ckpt>."
        )
